...
struct messageData
{
	...
	void writeInfoToFileWithName(string &name) const
	{
		ofstream file (BINARY_DIR name, ios::binary | ios::app);
...
string fileNameFromStockName(string &message)
{
	string finalName = "/output_";
	string endOfFile = ".txt";
	finalName.append(message.append(endOfFile));
...
int main(int argc, char* argv[])
{
	...
	if (inputFile.is_open())
	{
		...
		while (curPos < fileSize)
		{
			...
			newData->writeInfoToFileWithName(fileNameFromStockName(newData->message));
			delete newData;
